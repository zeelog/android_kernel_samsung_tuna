...
static int omap_sr_vmin_store(void *data, u64 val)
{
	...
	lowceiling=LOWCEILING;
        ...
        if (srtype==1) vddmax=sr_info->voltdm->pmic->uv_to_vsel(OMAP4460_VP_MPU_VLIMITTO_VDDMAX);
        else if (srtype==2) vddmax=sr_info->voltdm->pmic->uv_to_vsel(OMAP4460_VP_IVA_VLIMITTO_VDDMAX);
        else vddmax=sr_info->voltdm->pmic->uv_to_vsel(OMAP4460_VP_CORE_VLIMITTO_VDDMAX);
